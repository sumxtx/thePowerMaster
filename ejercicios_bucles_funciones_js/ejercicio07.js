...
function sortByGenre(list)
{
  const sortedList = {};
  for(ele of list)
  {
    if(!sortedList[ele.genre])
    {
      sortedList[ele.genre] = [];
      sortedList[ele.genre].push(ele.title);
    }
    else if(sortedList[ele.genre])
    {
      sortedList[ele.genre].push(ele.title);
    }
  }
...
console.log(sortByGenre(tracks));
